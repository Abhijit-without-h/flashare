...
async function scanBluetooth() {
    if (!navigator.bluetooth) {
        showToast('Bluetooth not supported in this browser', 'warning');
        showToast('Please scan the QR code on your laptop instead', 'info');
        return;
    }

    try {
        showToast('Scanning for Flashare devices...', 'info');
        ...
        showToast(`Found: ${device.name || 'Unknown Device'}`, 'success');

        // In a full implementation, we would:
        // 1. Connect to the GATT server
        // 2. Read the characteristic containing the server URL
        // 3. Navigate to that URL

        const server = await device.gatt.connect();
        showToast('Connected! Reading server info...', 'info');

    } catch (error) {
        if (error.name === 'NotFoundError') {
            showToast('No Bluetooth device selected', 'warning');
        } else {
            showToast('Bluetooth error: ' + error.message, 'error');
            showToast('Please use the QR code instead', 'info');
        }
    }
}
...
async function init() {
    // Load initial data
    try {
        const [filesData, status] = await Promise.all([
            fetchFiles(),
            fetchStatus(),
        ]);
    ...
    elements.scanBleBtn.addEventListener('click', scanBluetooth);
